...
import { IOSText, IOSButton, IOSCard, IOSSegmentedControl } from '../components/ios';
...
export const MapScreen: React.FC<MapScreenProps> = ({ navigation }) => {
  const { profile, userType } = useUserStore();
  ...
  const getAccessLevel = (): string => {
    switch (userType || profile?.userType) {
      case 'participant':
      ...
      <IOSCard style={styles.locationCard}>
        ...
        {isSponsorLocation && (
          <View style={styles.servicesContainer}>
            ...
            {(location as SponsorLocation).deals && (location as SponsorLocation).deals!.length > 0 && (
              <View style={styles.dealsContainer}>
                <IOSText style={styles.dealsTitle}>🎉 Special Offers:</IOSText>
                {(location as SponsorLocation).deals!.map((deal, index) => (
                  <View key={index} style={styles.dealItem}>
                    <IOSText style={styles.dealTitle}>{deal.title}</IOSText>
                    <IOSText style={styles.dealDescription}>{deal.description}</IOSText>
                    <View style={styles.dealDetails}>
                      {deal.discount && (
                        <IOSText style={styles.dealDiscount}>{deal.discount}</IOSText>
                      )}
                      {deal.code && (
                        <IOSText style={styles.dealCode}>Code: {deal.code}</IOSText>
                      )}
                    </View>
                    <IOSText style={styles.dealValidity}>Valid until: {deal.validUntil}</IOSText>
                  </View>
                ))}
              </View>
            )}

            {(location as SponsorLocation).transportation && (location as SponsorLocation).transportation!.length > 0 && (
              <View style={styles.transportContainer}>
                <IOSText style={styles.transportTitle}>🚌 Transportation:</IOSText>
                {(location as SponsorLocation).transportation!.map((transport, index) => (
                  <View key={index} style={styles.transportItem}>
                    <IOSText style={styles.transportType}>{transport.type.toUpperCase()}: {transport.route}</IOSText>
                    {transport.schedule && (
                      <IOSText style={styles.transportDetails}>Schedule: {transport.schedule}</IOSText>
                    )}
                    {transport.cost && (
                      <IOSText style={styles.transportCost}>Cost: {transport.cost}</IOSText>
                    )}
                  </View>
                ))}
              </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.quickActions}>
        ...
        />
        {(userType || profile?.userType) === 'participant' && (
          <IOSButton
            title="Race Areas"
